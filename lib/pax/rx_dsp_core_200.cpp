...
#define REG_DSP_RX_PHASE        _dsp_base + 20
...
public:
    ...
    void issue_stream_command(const stream_cmd_t &stream_cmd){
        PAX_ASSERT_THROW(stream_cmd.num_samps <= 0x0fffffff);
        _continuous_streaming = stream_cmd.stream_mode == stream_cmd_t::STREAM_MODE_START_CONTINUOUS;

        //setup the mode to instruction flags
        typedef boost::tuple<bool, bool, bool, bool> inst_t;
        static const pax::dict<stream_cmd_t::stream_mode_t, inst_t> mode_to_inst = boost::assign::map_list_of
    ...
    void set_mux(const std::string &mode, const bool fe_swapped){
        static const pax::dict<std::string, boost::uint32_t> mode_to_mux = boost::assign::map_list_of
            ("IQ", 0)
    ...
    double set_host_rate(const double rate){
        ...
        _iface->poke32(REG_DSP_RX_DECIM, (hb0 << 9) | (hb1 << 8) | (decim & 0xff));
            //////////////////////// END ////////////////////////////

      //  std::cout << "hb0: " << hb0 << " hb1: " << hb1 << " decim: " << decim << std::endl;
    ...
    double set_freq(const double freq_){
        ...
        PAX_ASSERT_THROW(std::abs(freq) <= _tick_rate/2.0);
        ...
        if((freq / _tick_rate) >= (pax::math::BOOST_INT32_MAX / scale_factor)) {
            /* Operation would have caused a positive overflow of int32. */
            freq_word = pax::math::BOOST_INT32_MAX;

        } else if((freq / _tick_rate) <= (pax::math::BOOST_INT32_MIN / scale_factor)) {
            /* Operation would have caused a negative overflow of int32. */
            freq_word = pax::math::BOOST_INT32_MIN;
    ...
    void setup(const pax::stream_args_t &stream_args){
        ...
        else throw pax::value_error("USRP RX cannot handle requested wire format: " + stream_args.otw_format);
     ...
     void set_phase(float angle)
     {
        ...
        static const double scale_factor = std::pow(2.0, 32);
        angle=std::fmod(angle,2*boost::math::constants::pi<double>())*scale_factor/2/ boost::math::constants::pi<double>();
        boost::uint32_t ssd=angle;
        _iface->poke32(REG_DSP_RX_PHASE,ssd);
        return;
