 ...
 **********************************************************************/
struct pax_wsa_control
{
    pax_wsa_control(void)
    {
        WSADATA wsaData;
...
public:
        ...
        : _sock_fd(sock_fd), _frame_size(frame_size)
    {
        ...
        PAX_ASSERT_THROW(_overlapped.hEvent != WSA_INVALID_EVENT);
...
public:
        ...
        , _next_send_buff_index(0)
    {
        ...
        static pax_wsa_control pax_wsa; // makes wsa start happen via lazy initialization

        PAX_ASSERT_THROW(_num_send_frames <= WSA_MAXIMUM_WAIT_EVENTS);
        ...
        PAX_LOGGER_TRACE("UDP") << boost::format("Local WSA UDP socket endpoint: %s:%s")
...
{
    PAX_LOGGER_DEBUG("UDP") << boost::format(
    ...
    const device_addr_t& hints)
{
    ...
    if (xport_params.recv_buff_size == 0) {
        PAX_LOG_TRACE("UDP", "Using default value for recv_buff_size");
    ...
    if (xport_params.send_buff_size == 0) {
        PAX_LOG_TRACE("UDP", "default_buff_args has no send_buff_size");
