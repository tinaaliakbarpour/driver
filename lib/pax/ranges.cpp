...
{
    if (stop < start){
        throw pax::value_error("cannot make range where stop < start");
    }
}
...
void check_meta_range_monotonic(const meta_range_t &mr){
    if (mr.empty()){
        throw pax::value_error("meta-range cannot be empty");
    }
    for (size_t i = 1; i < mr.size(); i++){
        if (mr.at(i).start() < mr.at(i-1).stop()){
            throw pax::value_error("meta-range is not monotonic");
        }
    }
}
