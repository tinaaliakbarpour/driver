...
constexpr char LOG_THREAD_NAME[]          = "pax_log";
constexpr char LOG_THREAD_NAME_FP[]       = "pax_log_fastpath";
constexpr char LOG_THREAD_NAME_FP_DUMMY[] = "pax_log_fp_dummy";

std::string verbosity_color(const pax::log::severity_level& level)
{
    static const bool tty = pax::is_a_tty(2); // is stderr a tty?
    if (!tty) {
        return "";
    }
    switch (level) {
...
std::string verbosity_name(const pax::log::severity_level& level)
{
    switch (level) {
        case (pax::log::trace):
            return "TRACE";
 ...
 * Logger backends
 **********************************************************************/
void console_log(const pax::log::logging_info& log_info)
{
    std::ostringstream log_buffer;
    log_buffer
#ifdef PAX_LOG_CONSOLE_COLOR
        << verbosity_color(log_info.verbosity)
#endif
#ifdef PAX_LOG_CONSOLE_TIME
        << "[" << pt::to_simple_string(log_info.time) << "] "
#endif
#ifdef PAX_LOG_CONSOLE_THREAD
        << "[0x" << log_info.thread_id << "] "
#endif
#ifdef PAX_LOG_CONSOLE_SRC
        << "[" << path_to_filename(log_info.file) << ":" << log_info.line << "] "
#endif
        << "[" << verbosity_name(log_info.verbosity) << "] "
        << "[" << log_info.component << "] "
#ifdef PAX_LOG_CONSOLE_COLOR
        << verbosity_color(pax::log::off) // This will reset colors
#endif
...
public:
    ...
    void log(const pax::log::logging_info& log_info)
    {
...
#define PAX_CONSOLE_LOGGER_KEY "console"
#define PAX_FILE_LOGGER_KEY "file"

class log_resource
{
public:
    pax::log::severity_level global_level;

    log_resource(void)
        : global_level(pax::log::trace)
        , _exit(false)
        ,
        ...
        _log_queue(10)
    {
...
#ifdef PAX_LOG_MIN_LEVEL
        this->global_level =
            _get_log_level(BOOST_STRINGIZE(PAX_LOG_MIN_LEVEL), this->global_level);
#endif
        // allow override from environment variables
        const char* log_level_env = std::getenv("PAX_LOG_LEVEL");
        ...
        if (enable_fastpath) {
            ...
            pax::set_thread_name(_pop_fastpath_task.get(), LOG_THREAD_NAME_FP);
        } else {
            _pop_fastpath_task = std::make_shared<std::thread>(
                std::thread([this]() { this->pop_fastpath_dummy_task(); }));
            pax::set_thread_name(_pop_fastpath_task.get(), LOG_THREAD_NAME_FP_DUMMY);
    ...
    ~log_resource(void)
    {
        ...
        auto final_message    = pax::log::logging_info(pt::microsec_clock::local_time(),
            pax::log::trace,
            __FILE__,
            __LINE__,
            "LOGGING",
...
#    endif
    ...
    void push(const pax::log::logging_info& log_info)
    {
    ...
    void _handle_log_info(const pax::log::logging_info& log_info)
    {
...
#    endif // BOOST_MSVC
    ...
    void add_logger(const std::string& key, pax::log::log_fn_t logger_fn)
    {
        std::lock_guard<std::mutex> l(_logmap_mutex);
        _loggers[key] = level_logfn_pair{global_level, logger_fn};
    }

    void set_log_level(const std::string& key, const pax::log::severity_level level)
    {
        std::lock_guard<std::mutex> l(_logmap_mutex);
...
#endif
    pax::log::severity_level _get_log_level(
        const std::string& log_level_str, const pax::log::severity_level& previous_level)
    {
        if (std::isdigit(log_level_str[0])) {
            const pax::log::severity_level log_level_num =
                pax::log::severity_level(std::stoi(log_level_str));
            if (log_level_num >= pax::log::trace and log_level_num <= pax::log::fatal) {
...
#define if_loglevel_equal(name) else if (log_level_str == #name) return pax::log::name
    ...
    {
#ifndef PAX_LOG_CONSOLE_DISABLE
        pax::log::severity_level console_level = pax::log::trace;
#    ifdef PAX_LOG_CONSOLE_LEVEL
        console_level =
            _get_log_level(BOOST_STRINGIZE(PAX_LOG_CONSOLE_LEVEL), console_level);
#    endif
        const char* log_console_level_env = std::getenv("PAX_LOG_CONSOLE_LEVEL");
        if (log_console_level_env != NULL && log_console_level_env[0] != '\0') {
            console_level = _get_log_level(log_console_level_env, console_level);
        }
        _loggers[PAX_CONSOLE_LOGGER_KEY] = level_logfn_pair{console_level, &console_log};
#endif
    }

    void _setup_file_logging()
    {
        pax::log::severity_level file_level = pax::log::trace;
        std::string log_file_target;
#if defined(PAX_LOG_FILE_LEVEL)
        file_level = _get_log_level(BOOST_STRINGIZE(PAX_LOG_FILE_LEVEL), file_level);
#endif
#if defined(PAX_LOG_FILE)
        log_file_target = BOOST_STRINGIZE(PAX_LOG_FILE);
#endif
        const char* log_file_level_env = std::getenv("PAX_LOG_FILE_LEVEL");
        if (log_file_level_env != NULL && log_file_level_env[0] != '\0') {
            file_level = _get_log_level(log_file_level_env, file_level);
        }
        const char* log_file_env = std::getenv("PAX_LOG_FILE");
        ...
        if (!log_file_target.empty()) {
            ...
            _loggers[PAX_FILE_LOGGER_KEY] = level_logfn_pair{file_level,
                [F](const pax::log::logging_info& log_info) { F->log(log_info); }};
        }
    }

    void _publish_log_msg(const std::string& msg,
        const pax::log::severity_level level = pax::log::info,
        const std::string& component         = "LOGGING")
    {
        if (level < global_level) {
            return;
        }
        auto log_msg    = pax::log::logging_info(pt::microsec_clock::local_time(),
            level,
            __FILE__,
            __LINE__,
            component,
    ...
    using level_logfn_pair = std::pair<pax::log::severity_level, pax::log::log_fn_t>;
    std::map<std::string, level_logfn_pair> _loggers;
#ifndef PAX_LOG_FASTPATH_DISABLE
    pax::transport::bounded_buffer<std::string> _fastpath_queue;
#endif
    pax::transport::bounded_buffer<pax::log::logging_info> _log_queue;
};

PAX_SINGLETON_FCN(log_resource, log_rs);

/***********************************************************************
 * The logger object implementation
 **********************************************************************/
pax::_log::log::log(const pax::log::severity_level verbosity,
...
{
    if (_log_it) {
        this->_log_info = pax::log::logging_info(pt::microsec_clock::local_time(),
            verbosity,
            file,
            line,
            component,
...
#ifndef PAX_LOG_FASTPATH_DISABLE
void pax::_log::log_fastpath(const std::string& msg)
{
    log_rs().push_fastpath(msg);
}
#else
void pax::_log::log_fastpath(const std::string&)
{
    // nop
}
#endif
 ...
 * Public API calls
 **********************************************************************/
void pax::log::add_logger(const std::string& key, log_fn_t logger_fn)
{
    log_rs().add_logger(key, logger_fn);
}

void pax::log::set_log_level(pax::log::severity_level level)
{
    log_rs().global_level = level;
}

void pax::log::set_logger_level(const std::string& key, pax::log::severity_level level)
{
    log_rs().set_log_level(key, level);
}

void pax::log::set_console_level(pax::log::severity_level level)
{
    set_logger_level(PAX_CONSOLE_LOGGER_KEY, level);
}

void pax::log::set_file_level(pax::log::severity_level level)
{
    set_logger_level(PAX_FILE_LOGGER_KEY, level);
}
