...
public:
    ...
    pax::time_spec_t get_time_now(void){
        ...
        throw pax::runtime_error("time64_core_200: get time now timeout");
    }

    pax::time_spec_t get_time_last_pps(void){
        for (size_t i = 0; i < 3; i++){ //special algorithm because we cant read 64 bits synchronously
        ...
        throw pax::runtime_error("time64_core_200: get time last pps timeout");
    }

    void set_time_now(const pax::time_spec_t &time){
    ...
    void set_time_next_pps(const pax::time_spec_t &time){
        const boost::uint64_t ticks = time.to_ticks(_tick_rate);
        _iface->poke32(REG_TIME64_TICKS_LO, boost::uint32_t(ticks >> 0));
...
time64_core_200::sptr time64_core_200::make(spi_wb_iface::sptr iface, const size_t base, const readback_bases_type &readback_bases, const size_t mimo_delay_cycles){
