    ...
    , _send_memory_pool(buffer_pool::make(params.num_send_frames, params.send_frame_size))
{
    ...
    PAX_LOGGER_TRACE("UDP") << boost::format("Created UDP link to %s:%s") % addr % port;
    PAX_LOGGER_TRACE("UDP") << boost::format("Local UDP socket endpoint: %s:%s")
...
{
    PAX_ASSERT_THROW(params.num_recv_frames != 0);
    PAX_ASSERT_THROW(params.num_send_frames != 0);
    PAX_ASSERT_THROW(params.recv_frame_size != 0);
    PAX_ASSERT_THROW(params.send_frame_size != 0);
    PAX_ASSERT_THROW(params.recv_buff_size != 0);
    PAX_ASSERT_THROW(params.send_buff_size != 0);
