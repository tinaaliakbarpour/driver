...
void ad9361_device_t::_calibrate_lock_bbpll()
{
    ...
    PAX_ASSERT_THROW(_io_iface->peek8(0x03f)==1);
...
double ad9361_device_t::_calibrate_baseband_rx_analog_filter(double req_rfbw)
{
    ...
    {
        PAX_LOGGER_WARNING("AD9361") << "baseband bandwidth too large for current sample rate. Setting bandwidth to: "<<_baseband_bw;
...
{
    PAX_LOGGER_INFO("AD9361") << boost::format("[ad9361_device_t::_tune_bbvco] rate=%.10f\n") % rate;
...
void ad9361_device_t::set_rx_lo_source(lo_mode_t extint){
    ...
    if (_use_ext_tx_lo == EXTERNAL_LO && extint == EXTERNAL_LO)
        PAX_LOGGER_WARNING("AD9361") << " both Rx/Tx are set to external LO. In this mode one single LO frequency is possible.";
...
double ad9361_device_t::set_clock_rate(const double req_rate)
{
    ...
    /* PAX has a habit of requesting the same rate like four times when it
...
void ad9361_device_t::ensm_force_alert()
{
    ...
    if (ensm_state == 0x05) {
        PAX_LOGGER_WARNING("AD9361")  << "Nothing to do, device is already in alert state";
        return;
    }
...
void ad9361_device_t::ensm_force_fdd()
{
    uint8_t ensm_state = _io_iface->peek8(0x017) & 0x0f;
    if (ensm_state == 0xA) {
        PAX_LOGGER_WARNING("AD9361") << "Nothing to do, device is already in fdd state" ;
        return;
    }
