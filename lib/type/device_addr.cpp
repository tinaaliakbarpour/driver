...
device_addr_t::device_addr_t(const std::string &args){
    BOOST_FOREACH(const std::string &pair, tokenizer(args, arg_delim)){
        ...
        {
        }//throw pax::value_error("invalid args string: "+args); //otherwise error
    }
}
...
device_addrs_t pax::separate_device_addr(const device_addr_t &dev_addr){
...
device_addr_t pax::combine_device_addrs(const device_addrs_t &dev_addrs){
