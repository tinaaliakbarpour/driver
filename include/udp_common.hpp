...
#if defined(PAX_PLATFORM_MACOS) || defined(PAX_PLATFORM_BSD)
 ...
 */
PAX_INLINE bool wait_for_recv_ready(int sock_fd, int32_t timeout_ms)
{
#ifdef PAX_PLATFORM_WIN32 // select is more portable than poll unfortunately
...
PAX_INLINE socket_sptr open_udp_socket(
...
PAX_INLINE size_t recv_udp_packet(
    int sock_fd, void* mem, size_t frame_size, int32_t timeout_ms)
{
    ...
    if (wait_for_recv_ready(sock_fd, timeout_ms)) {
        len = pax::narrow_cast<ssize_t>(::recv(sock_fd, (char*)mem, frame_size, 0));
        if (len == 0) {
            throw pax::io_error("socket closed");
        }
        if (len < 0) {
            throw pax::io_error(
...
PAX_INLINE void send_udp_packet(int sock_fd, void* mem, size_t len)
{
    ...
    while (true) {
        const ssize_t ret =
            pax::narrow_cast<ssize_t>(::send(sock_fd, (const char*)mem, len, 0));
        if (ret == ssize_t(len))
            break;
        ...
        PAX_ASSERT_THROW(ret == ssize_t(len));
    }
}
...
PAX_INLINE size_t resize_udp_socket_buffer_with_warning(
...
inline link_params_t calculate_udp_link_params(
    const pax::transport::link_type_t link_type,
    const size_t send_mtu,
    const size_t recv_mtu,
    const link_params_t& default_link_params,
    const pax::device_addr_t& device_args,
    const pax::device_addr_t& link_args)
{
    ...
    if (link_type == link_type_t::CTRL) {
        ...
        // configured.
        link_params.num_recv_frames =
            pax::rfnoc::CMD_FIFO_SIZE / pax::rfnoc::MAX_CMD_PKT_SIZE;
