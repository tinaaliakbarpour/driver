...
#ifndef INCLUDED_PAX_TRANSPORT_UDP_BOOST_ASIO_LINK_HPP
#define INCLUDED_PAX_TRANSPORT_UDP_BOOST_ASIO_LINK_HPP
...
private:
    ...
    PAX_INLINE size_t get_recv_buff_derived(frame_buff& buff, int32_t timeout_ms)
    {
        return recv_udp_packet(_sock_fd, buff.data(), get_recv_frame_size(), timeout_ms);
    }

    PAX_INLINE void release_recv_buff_derived(frame_buff& /*buff*/)
    {
        // No-op
    }

    // Methods called by send_link_base
    PAX_INLINE bool get_send_buff_derived(
        frame_buff& /*buff*/, int32_t /*timeout_ms*/)
    {
        return true;
    }

    PAX_INLINE void release_send_buff_derived(frame_buff& buff)
    {
...
#endif /* INCLUDED_PAX_TRANSPORT_UDP_BOOST_ASIO_LINK_HPP */
