...
#ifndef INCLUDED_LIBPAX_TRANSPORT_SUPER_RECV_PACKET_HANDLER_HPP
#define INCLUDED_LIBPAX_TRANSPORT_SUPER_RECV_PACKET_HANDLER_HPP
...
PAX_INLINE uint32_t get_context_code(
...
public:
     ...
     ******************************************************************/
    PAX_INLINE size_t recv(const pax::rx_streamer::buffs_type& buffs,
...
private:
     ...
     ******************************************************************/
    PAX_INLINE packet_type get_and_process_single_packet(const size_t index,
...
// 2) check for sequence errors
        ...
        if (expected_packet_count != info.ifpi.packet_count) {
            // PAX_LOGGER_INFO("STREAMER") << "expected: " << expected_packet_count << "
     ...
     ******************************************************************/
    PAX_INLINE void alignment_check(const size_t index, buffers_info_type& info)
    {
    ...
    PAX_INLINE void get_aligned_buffs(double timeout)
    {
        ...
        while (curr_info.indexes_todo.any()) {
            ...
            catch (const pax::value_error& e) {
                PAX_LOGGER_ERROR("STREAMER")
                    << boost::format(
     ...
     ******************************************************************/
    PAX_INLINE size_t recv_one_packet(const pax::rx_streamer::buffs_type& buffs,
        const size_t nsamps_per_buff,
        pax::rx_metadata_t& metadata,
        const double timeout,
    ...
    inline void convert_to_out_buff(const size_t index)
    {
        ...
        memcpy(out_buffs[0],info.copy_buff,_convert_bytes_to_copy);
...
}}} // namespace pax::transport::sph

#endif /* INCLUDED_LIBPAX_TRANSPORT_SUPER_RECV_PACKET_HANDLER_HPP */
